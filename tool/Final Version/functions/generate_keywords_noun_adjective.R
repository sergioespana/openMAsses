...
require(NLP)
require(openNLP)
# x <- "the female executive ratio is defined by the ratio of female executives to male executives whereas the happy executives are excluded."

# input: a sentence string that describes a topic
# output: a list with keywords that uses nouns and adjectives tuple

ozp_generate_keywords_nouns_adjectives <- function(input) {


    input <- as.String(input)
    # Before POS tagging, we need to do Sentence annotation followed by word annotation
    wordAnnotation <- annotate(input, list(Maxent_Sent_Token_Annotator(), Maxent_Word_Token_Annotator()))

    # POS tag the words & extract the "words" from the output
    POSAnnotation <- annotate(input, Maxent_POS_Tag_Annotator(), wordAnnotation)
    POSwords <- subset(POSAnnotation, type == "word")
    # Extract the tags from the words
    tags <- sapply(POSwords$features, '[[', "POS")

    # Create a data frame with words and tags
    tokenizedAndTagged <- data.frame(Tokens = input[POSwords], Tags = tags)
